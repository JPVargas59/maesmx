...
export class DatabaseService {
  ...
  saveUserInfo(user: UserInfo): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.afs.doc<UserInfo>('users/' + id).set(user, {merge: true}).then(() => {
        resolve();
      }).catch(error => {
        reject(error);
      });
  ...
  updateUserInfo(uid: any, data: any): Promise<void> {
    return new Promise((resolve, reject) => {
      this.afs.doc('users/' + uid).update(data).then(() => {
        resolve();
      }).catch(error => {
        reject(error);
      });
  ...
  addRequest(userInfo: UserInfo, subject: { id: string, name: string }, comment: string | undefined): Promise<string> {
    return new Promise((resolve, reject) => {
        this.afs.collection<HelpRequest>('requests').add({
          userInfo: userInfo,
          subject: {
            id: subject.id,
            name: subject.name
  ...
  getHelpRequest(requestId: string): Observable<HelpRequest> {
    return this.afs.doc<any>(`requests/${requestId}`).valueChanges() as Observable<HelpRequest>;
  }

  getActiveRequests(): Observable<HelpRequest[]> {
    return this.afs.collection<HelpRequest>('requests', ref => ref.where('status', '==', Status.PENDING)).valueChanges({idField: 'id'}) as Observable<HelpRequest[]>;
  }

  updateHelpRequest(requestId: string, data: HelpRequest): Promise<void> {
    data.updatedAt = this.timestamp;
    return new Promise((resolve, reject) => {
        this.afs.doc(`requests/${requestId}`).update(data).then(() => {
          resolve();
        })
          .then(async () => {
            // add 1 to requests counter in school document
            if (data.status === Status.ACCEPTED) {
              await this.afs.doc(`schools/${localStorage.getItem('email')!.split('@')[1]}`).update({
  ...
  addPeerSession(peer: PeerInfo): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.afs.collection<PeerSession>(`users/${peer.uid}/sessions`).add(session)
        .then(s => {
          this.afs.doc<PeerInfo>(`users/${peer.uid}`).update({
            activeSession: {
              ...session,
              id: s.id
            },
  ...
  getPeerSession(peerId: string, sessionId: string): Observable<PeerSession> {
    return this.afs.doc<any>(`users/${peerId}/sessions/${sessionId}`).valueChanges({idField: 'id'}) as Observable<PeerSession>;
  }

  getCurrentUserActiveSession(): Observable<PeerSession> {
    return this.afs.doc<any>(`users/${localStorage.getItem('uid')}/sessions/${localStorage.getItem('activeSession')}`).valueChanges({idField: 'id'}) as Observable<PeerSession>;
  }

  endSession(peerId: string, session: PeerSession): Promise<void> {
    session.endTime = this.timestamp;
    const endTimeDate = new Date();
    session.totalTime = (endTimeDate.getTime() - session.startTime.toDate().getTime()) / 1000 / 60;
    ...
    return new Promise((resolve, reject) => {
        this.afs.doc(`users/${peerId}/sessions/${session.id}`).update(session)
          .then((s) => {
            this.afs.doc(`users/${peerId}`).update({
              activeSession: this.deleteField,
              totalTime: session.status == PeerSessionStatus.ACCEPTED ?
                DatabaseService.incrementField(session.totalTime!) : DatabaseService.incrementField(0),
            })
            resolve();
          }).catch(error => {
          reject(error);
  ...
  getSettings(): Observable<Settings> {
    const emailDomain = localStorage.getItem('email')!.split('@')[1];
    return this.afs.doc<any>(`schools/${emailDomain}`).valueChanges() as Observable<Settings>;
  }

  saveUserSchedule(peerInfo: PeerInfo): Promise<void> {

    return new Promise((resolve, reject) => {
      if (!peerInfo.weekSchedule) reject('Week schedule is empty, missing hours');
      if (!peerInfo.subjects) reject('Subjects is empty, missing subjects');
      for (let weekDay in peerInfo.weekSchedule) {
        // @ts-ignore
        if (!peerInfo.weekSchedule[weekDay]) reject(`${weekDay} is empty, missing hours`);
        for (let subject of peerInfo.subjects!) {
          this.afs.doc(`schedule/${weekDay}/${subject.id}/${peerInfo.uid}/`).set({
            // @ts-ignore
  ...
  getSchedules(day: WeekDays, subject: string): Observable<PeerSchedule[]> {
    return this.afs.collection<PeerSchedule>(`schedule/${day}/${subject}`, ref => ref.orderBy('userInfo.name')).valueChanges({idField: 'id'}) as Observable<PeerSchedule[]>;
  }

  getUser(uid: string): Observable<PeerInfo> {
    return this.afs.doc<any>(`users/${uid}`).valueChanges({idField: 'id'}) as Observable<PeerInfo>;
  }
